...
function formatCNPJ(v?: string | null) {
  ...
  return d
    .slice(0, 14)
    .replace(/(\d{2})(\d{3})(\d{3})(\d{0,4})(\d{0,2})/, (_, a, b, c, d, e) =>
      e ? `${a}.${b}.${c}/${d}-${e}` : `${a}.${b}.${c}/${d}`
    );
}
    ...
    <main style={{ padding: 24, display: 'grid', gap: 16, maxWidth: 1100 }}>
      ...
      <section style={card}>
        ...
        >
          ...
          />
          <CPFInput name="cpf" placeholder="CPF (opcional)" />
          <CNPJInput name="cnpj" placeholder="CNPJ (opcional)" />
          <input
            name="email"
                  ...
                  <details style={{ display: 'inline-block', marginRight: 8 }}>
                    ...
                    <div style={{ paddingTop: 8 }}>
                      ...
                      >
                        ...
                        <CPFInput name="cpf" defaultValue={c.cpf ?? ''} />
                        <CNPJInput name="cnpj" defaultValue={c.cnpj ?? ''} />
                        <input
                          name="email"
...
function CPFInput({ name, defaultValue = '', placeholder = '' }) {
  useEffect(() => {
...
function CNPJInput({ name, defaultValue = '', placeholder = '' }) {
  useEffect(() => {
    const input = document.querySelector(`input[name="${name}"]`);
    if (!input) return;
  ...
  return (
    <input
      name={name}
      defaultValue={formatCNPJ(defaultValue)}
