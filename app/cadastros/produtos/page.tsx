 ...
 */
function pickMinMaxByTipo(vincs: VincForn[], tipo: 'PRODUTO' | 'SERVICO' | 'AMBOS' | null | undefined) {
...
export default async function Page() {
  ...
  // vínculos (Fornecedor ↔ Produto) com preços de materiais e mão de obra
  ...
  return (
    <main style={{ padding: 24, display: 'grid', gap: 16, maxWidth: 1200 }}>
      ...
      {/* Mostrar "Salvar" só quando o editor (details) estiver aberto */}
      <style>{`
      ...
      <section>
        <table
          style={{
            ...
            background: '#fff',
            tableLayout: 'fixed',                 // trava layout
          }}
        >
          <colgroup>
            ...
            <col style={{ width: 70 }} />         {/* UM */}
            <col style={{ width: 140 }} />        {/* Categoria */}
            <col style={{ width: 110 }} />        {/* P1 */}
            <col style={{ width: 180 }} />        {/* Forn P1 */}
            <col style={{ width: 110 }} />        {/* P3 */}
            <col style={{ width: 180 }} />        {/* Forn P3 */}
            <col style={{ width: 160 }} />        {/* Ações */}
          </colgroup>
          ...
          <tbody>
            {produtos.map(p => {
              ...
              return (
                <tr key={p.id}>
                  <td style={td}>{p.id}</td>
                  <td style={{ ...td }} className="ellipsis" title={p.nome}>{p.nome}</td>
                  <td style={td}>{tipoLabel(p.tipo)}</td>
                  <td style={td}>{p.unidade?.sigla}</td>
                  <td style={{ ...td }} className="ellipsis" title={p.categoria ?? ''}>{p.categoria ?? '—'}</td>

                  <td style={{ ...td, textAlign: 'right' }}>{min ? money(min.preco) : '—'}</td>
                  <td style={{ ...td }} className="ellipsis" title={min?.fornecedor ?? ''}>{min?.fornecedor ?? '—'}</td>
                  <td style={{ ...td, textAlign: 'right' }}>{max ? money(max.preco) : '—'}</td>
                  <td style={{ ...td }} className="ellipsis" title={max?.fornecedor ?? ''}>{max?.fornecedor ?? '—'}</td>
                    ...
                    <details style={{ display: 'inline-block', marginRight: 8 }}>
                      ...
                      <div style={{ paddingTop: 8 }}>
                        <AutoCloseForm
                          ...
                          style={{ display: 'grid', gap: 8, gridTemplateColumns: '1fr 140px 160px 1fr', maxWidth: 900 }}
                        >
                          ...
                          {/* Tipo */}
                          <select name="tipo" defaultValue={String(p.tipo ?? 'AMBOS')} required style={{ ...input, height: 36 }}>
                            <option value="PRODUTO">Produto</option>
                            <option value="SERVICO">Serviço</option>
                            <option value="AMBOS">Ambos</option>
                          </select>
                          ...
                          <input name="categoria" defaultValue={p.categoria ?? ''} placeholder="Categoria" style={input} />
                        </AutoCloseForm>
                      </div>
                    </details>
            ...
            {produtos.length === 0 && (
              <tr><td style={td} colSpan={10}>Nenhum produto/serviço cadastrado.</td></tr>
            )}
          </tbody>
        </table>
      </section>
