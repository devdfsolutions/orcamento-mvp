      ...
      <section>
        <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
          ...
          <tbody>
                  ...
                  <details>
                    ...
                    <div style={{ paddingTop: 8 }}>
                      ...
                      >
                        ...
                        <select name="fornecedorId" defaultValue={String(i.fornecedorId)} required style={input}>
                          {fornecedores.map(f => (
                            <option key={f.id} value={f.id}>{f.nome}</option>
                          ))}
                        </select>

                        <select name="unidadeId" defaultValue={String(i.unidadeId)} required style={input}>
                          {unidades.map(u => (
            ...
            {itens.length === 0 && (
              <tr><td style={td} colSpan={9}>Nenhum item adicionado ainda.</td></tr>
            )}
          </tbody>
          <tfoot>
            <tr style={{ borderTop: '2px solid #ccc' }}>
              <td colSpan={7} style={{ ...td, textAlign: 'right', fontWeight: 700 }}>Total</td>
              <td colSpan={2} style={{ ...td, textAlign: 'right', fontWeight: 700 }}>{money(total)}</td>
            </tr>
          </tfoot>
        </table>
      </section>
...
const btn: React.CSSProperties = {
  height: 30, padding: '0 12px', borderRadius: 8, border: '1px solid #ddd', background: '#f8f8f8', color: '#111', cursor: 'pointer',
};
const primaryBtn: React.CSSProperties = {
  height: 36, padding: '0 14px', borderRadius: 8, border: '1px solid #111', background: '#111', color: '#fff', cursor: 'pointer',
};
const dangerBtn: React.CSSProperties = {
  height: 30, padding: '0 10px', borderRadius: 8, border: '1px solid #f1d0d0', background: '#ffeaea', color: '#b40000', cursor: 'pointer',
};
const linkBtn: React.CSSProperties = {
  display: 'inline-block', padding: '8px 12px', border: '1px solid #ddd', borderRadius: 8, background: '#f8f8f8', textDecoration: 'none', color: '#111',
};
